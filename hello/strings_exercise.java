...
//	}
		...
		String s = "";
		s = input.next();
		shifts = input.nextInt();
		String s1 = "";
		for(int i=0;i<s.length();i++) {
			if(Character.isUpperCase(s.charAt(i))) {
				char ch = (char)(((int)s.charAt(i)+shifts-65)%26+65);
				s1 += ch;
			}else {
				char ch = (char)(((int)s.charAt(i)+shifts-97)%26+97);
				s1 += ch;
			}
		}
		...
		for(int i=0;i<s3.length();i++) {
			if(Character.isUpperCase(s3.charAt(i))) {
				char ch = (char)(((int)s3.charAt(i)+shifts1-65)%26+65);
				s2 += ch;
			}else {
				char ch = (char)(((int)s3.charAt(i)+shifts1-97)%26+97);
				s2+=ch;
			}
		}
